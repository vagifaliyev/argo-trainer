...
def normilize(array, value):
    norm = (value - min(array)) / (max(array) - min(array))
    return norm

def de_normalize(array, mean):
    out = - (mean * (max(array) - min(array)) - min(array))
    return out

def data_loader():
    # USER SELECTION DATA
    path = r'/Users/vagifaliyev/Desktop/ALL/DATA/' # use your path
    ...
    path = r'/Users/vagifaliyev/Desktop/ALL/csv_files/' # use your path
    ...
    for i in range(len(df_y.index)):
        ...
        x1 = normilize(PSAL, df_y['x1'][i])
        x2 = normilize(PSAL, df_y['x2'][i])
        y1 = normilize(TEMP, df_y['y1'][i])
        y2 = normilize(TEMP, df_y['y2'][i])
...
def main():
    ...
    for i in range(int(gen)):
        ...
        x1 = de_normalize(psal, x_1)
        x2 = de_normalize(psal, x_2)
        y1 = de_normalize(temp, y_1)
        y2 = de_normalize(temp, y_2)
        ...
        txt = "Salinity [PSU]: "+ str(x2) + " — " + str(x1) + '\n' + "Temperature [°C]: "+ str(y2) + " — " + str(y1) 
        plt.figtext(0.92, 0.15, txt, wrap=True, horizontalalignment='right', fontsize=9)
        # basemap mapping
        # t2 = time.time()
